...
import { Sparkles, Zap, TrendingUp, Target, ArrowDownCircle } from "lucide-react";

/**
 * [메인 히어로 세션 컴포넌트]
 * 상단 텍스트 메뉴 대신 메인 화면 칠판에 직관적인 대형 버튼 카드 형태로
 * 3가지 수학 학습 앱(일차방정식 랭킹, 정비례/반비례 그래프, 순서쌍 오목) 바로가기를 제공합니다.
 */
export default function Hero() {
  return (
    <section id="hero" className="w-full py-10 md:py-16 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      {/* 칠판 프레임 (아날로그 원목 테두리) */}
      <div className="relative wood-frame rounded-2xl bg-teal-900/90 p-6 sm:p-10 md:p-12 border-4 border-amber-900/80 shadow-2xl overflow-hidden">
        ...
        <div className="border-2 border-dashed border-teal-500/40 rounded-xl p-6 sm:p-8 flex flex-col items-center text-center space-y-6">
          ...
          <p className="font-dodum text-base sm:text-xl text-teal-100/90 max-w-2xl leading-relaxed">
            ...
            모든 학습 결과는 <strong>Supabase 클라우드 데이터베이스</strong>에 실시간 저장됩니다.
          </p>

          {/* === 메인 칠판 수학 앱 대형 버튼 카드 메뉴판 === */}
          <div className="w-full pt-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full text-left">
              ...
              <a
                href="#equation-game"
                className="group relative p-5 bg-teal-950/90 hover:bg-amber-950/80 rounded-2xl border-2 border-dashed border-chalk-yellow hover:border-amber-400 shadow-xl transition-all transform hover:-translate-y-1 cursor-pointer flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="p-2 bg-amber-500/20 text-chalk-yellow rounded-xl border border-dashed border-amber-400">
                      <Zap className="w-6 h-6 fill-chalk-yellow" />
                    </span>
                  ...
                  <p className="text-xs font-dodum text-teal-200/90 leading-relaxed">
                    60초 동안 일차방정식을 빠르게 풀고 Supabase 명예의 전당 랭킹 보드에 기록하세요!
                  </p>
                </div>
              ...
              {/* 버튼 3: (0,0) 중심 순서쌍 블라인드 오목 */}
              <a
                href="#blind-omok"
                className="group relative p-5 bg-teal-950/90 hover:bg-emerald-950/80 rounded-2xl border-2 border-dashed border-chalk-white hover:border-emerald-400 shadow-xl transition-all transform hover:-translate-y-1 cursor-pointer flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-mono font-bold text-emerald-300 bg-emerald-950 px-2 py-0.5 rounded border border-emerald-800">
                      (0,0) 원점 중심
                    </span>
                  </div>
        ...
        <div className="mt-6 pt-4 border-t-2 border-dashed border-teal-700/50 flex justify-between items-center text-xs text-teal-300/70 font-pen text-lg">
          ...
          <span>🧹 언제든 원하는 기능을 수정하고 추가해보세요!</span>
        </div>
